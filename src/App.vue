<template>
  ...
  <div class="flex flex-wrap">
    <div @click="toggleMenu" class="hidden bg-gray-100 sidebar-bg-close text-right p-3 px-5 ">
      <span>Close</span>
    </div>
    ...
    <div class="w-full lg:w-5/6">
      <div class="flex flex-wrap fixed top-0 right-0 bg-indigo-700 text-white w-full shadow-sm p-2 lg:px-6 z-40">
            ...
            <h1 class=" p-1 font-bold text-lg text-center flex flex-wrap">
                ...
                <svg @click="toggleMenu" class="w-6 block lg:hidden shadow-sm rounded-lg  text-white text-sm mr-2" id="i-menu" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 32 32" width="32" height="32" fill="none" stroke="currentcolor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2">
                    <path d="M4 8 L28 8 M4 16 L28 16 M4 24 L28 24" />
                </svg>
                
             
                <router-link class="flex font-semibold text-2xl tracking-tight ml-3" to="/">
                   <img class="border-solid border-2 border-indigo-100 rounded-full"  height="26px" src="@/assets/logo-bootwind-template-sm.png" alt="logo-bootwind-template">
      ...
      <router-view class="bg-white pt-16 p-3 min-h-screen"/>

          <!-- <div v-if="alert" class="p-2 px-3 bg-indigo-700 items-center text-indigo-100  rounded-lg flex  flex-wrap fixed top-0 right-0 max-w-md m-1 border-2 border-gray-100 shadow-lg lg:mr-8 lg:mt-8 z-50" >
                <span class="rounded-full bg-indigo-500 uppercase px-2 py-1 text-xs">Info</span>
                <span class="p-2 font-bold flex-auto">Selamat Malam BootWinder!
                      <svg @click="alert = false" class="bi bi-x-circle cursor-pointer float-right" width="1em" height="1em" viewBox="0 0 16 16" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
                ...
                <p class="p-2">
                    Terimakasih Sudah Subscribe Channel Ini
                </p>
                <p class="p-2 pt-0">
                    Semoga kita selalu diberi kesehatan agar bisa terus berkarya bersama bootwind template
                </p>
            </div> -->
    </div>
  </div>
...
export default {
  ...
  methods:{
    toggleMenu(){
      if(this.className == 'sidebar-active'){
         this.className = '';
      }else{
